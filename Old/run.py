...
def log_generation(song_data, instruments, patterns, output_dir: Path, timestamp: str):
    """Log the generation process and results."""
    log_file = output_dir / "Output-Text" / f"{song_data.title.replace(' ', '_')}_{timestamp}_log.txt"
...
def process_song(song_file: Path) -> None:
    ...
    try:
        ...
        output_file = output_dir / f"{song_file.stem}_{timestamp}.mid"
        ...
        for instrument in instruments:
            ...
            try:
                ...
                # Write individual MIDI file with timestamp
                output_file = output_dir / f"{song_data.title.replace(' ', '_')}_{instrument.__class__.__name__.lower()}_{timestamp}.mid"
                midi_gen.write(str(output_file))
                print(f"Generated MIDI file: {output_file}")
        ...
        log_generation(song_data, instruments, patterns, output_dir, timestamp)
        log_file = output_dir / "Output-Text" / f"{song_data.title.replace(' ', '_')}_{timestamp}_log.txt"
